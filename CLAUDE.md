...
`POST /shutdown` and `cuems-common`'s `cuems-cluster-poweroff` (the systemd `ExecStop` hook) both run the **same** two stage bodies and the **same** node selection, from `src/cuemspowerbridge/cluster_shutdown.py`. They orchestrate differently on purpose: the HTTP route runs the stages concurrently and then arms the relay and powers the controller off; the hook runs displays first (the nodes feed the projectors), probes liveness once and SSHes only the machines that answered, and does **neither** the relay nor the local poweroff — systemd is already doing that, and `/shutdown` ends by triggering that very transition, so a shared sequence would arm the relay twice. The hook reaches us as `"$venv_python" -m cuemspowerbridge.scripts.cluster_poweroff` — a module, not a `PATH` command, which keeps `cluster-poweroff.conf`'s interpreter override and its missing-bridge guard working. **One `flock` on `/run/cuems-power-bridge/shutdown.lock`** (tmpfiles rule shipped here) covers a whole sequence: the wrapper holds it across both stages, the daemon takes it too and **releases it before the local poweroff** so its own re-entrant transition is not refused. A **manual** `--force` run refuses while a project plays (`--while-playing` overrides); **a poweroff transaction never asks — the venue can always be powered off mid-show.**
...
- **Autoload vs node-boot race**: in a WoL recovery where the controller boots alone, autoload fires before nodes are up → engine load stalls after 120 s (armed never goes `yes`). A plain bridge restart then skips autoload. Recovery: `stop cuems-power-bridge; restart both engines; start cuems-power-bridge`. ⚠️ **The old symptom line is obsolete on engines carrying 869dr2kzh**: this used to read *"engine goes `project_status=none` but keeps broadcasting a stale 'loaded' over OSC"*. That contradiction was the engine's bug, not a race artifact — `project_status` collapsed loaded-but-not-playing into `none` while `/engine/status/load` correctly named the project. A current engine answers `loaded` and the two channels agree, so **`project_status=none` here now means what it says**: no project. Keep gating readiness on `armed`/`fully_ready()`, never on `project_loaded()` alone — a stalled load is genuinely `loaded`.
