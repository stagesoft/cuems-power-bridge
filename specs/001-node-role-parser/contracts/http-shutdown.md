...
| **3b** | machines would be targeted, **none addressable** — **`force` does NOT override** | **409** | `{"ok": false, "reason": "no_resolvable_nodes", "found": <n>, "unresolvable": [<uuid>, …]}` | **untouched** |
...
Every refusal body's message names the offending document or machine and a remedy
(FR-026). `detail` for `topology_unreadable` is one of: `settings_xml_missing`,
...
### `force=1` semantics — two effects, two non-effects
...
3. does **not** override `topology_unreadable` (Case 5);
4. does **not** override `no_resolvable_nodes` (Case 3b) — an unreachable machine cannot be
   commanded off by asserting harder. *Force overrides policy, never evidence.*

### Partial resolution

When some targets resolve and others do not, the shutdown **proceeds** (200) with those it
can reach, logs each unresolvable machine at ERROR, and reports
`node_selection.partial: true`. A monitor alerts on that field; the shutdown is not
refused, because leaving the venue powered on is the worse outcome (spec, *Partial
resolution*).
...
- `partial: true` means the selection was short of what the document implies — some
  machine could not be addressed (FR-025).
